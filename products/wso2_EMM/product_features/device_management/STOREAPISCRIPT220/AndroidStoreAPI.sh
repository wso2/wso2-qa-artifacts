...
#GEN MULTI SCOPED TOKEN 
MULTI_SCOPED_TOKEN=$(curl -k -d "grant_type=password&username=admin&password=admin&scope=device:android:operation:install-app device:android:enroll device:android:operation:uninstall-app" -H "Authorization: Basic $OAUTH_BASIC_TOKEN" -H "Content-Type: application/x-www-form-urlencoded" "https://$1:$2/oauth2/token" | cut -d "," -f1 | cut -c18-49) ;

echo "------------- $MULTI_SCOPED_TOKEN --------"
#REG DEVICE
curl -X POST -H "Authorization: Bearer $MULTI_SCOPED_TOKEN" -H "Content-Type: application/json" -d @payload3.json -k "https://$1:$2/api/device-mgt/android/v1.0/devices"  
...
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $MULTI_SCOPED_TOKEN" -d @payload5.json -k "https://$1:$2/api/device-mgt/android/v1.0/admin/devices/install-application" | grep "INSTALL_APPLICATION"

if [ $? -gt 0 ]; then
        echo  "$(date) - App remote install error." >> errorlog.log ;
fi

curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $MULTI_SCOPED_TOKEN" -d @payload6.json -k "https://$1:$2/api/device-mgt/android/v1.0/admin/devices/uninstall-application" | grep "UNINSTALL_APPLICATION"
